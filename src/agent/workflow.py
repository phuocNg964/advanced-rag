...
REWRITER_PROMPT = """You are a Query Preprocessor. Your output must be a JSON array of strings, nothing else.
...
class AgenticRAG:
    def __init__(self):
        ...
        if _settings.use_local_llm:
            logger.info(f"Using local LLM via Ollama: {_settings.ollama_model}")
            self.llm_rag = ChatOllama(
                model=_settings.ollama_model,
                base_url=_settings.ollama_host,
                temperature=0.3,
            )
            self.llm_rewriter = ChatOllama(
                model=_settings.ollama_model,
                base_url=_settings.ollama_host,
                temperature=0.3,
            )
            ...
            if not _settings.gemini_api_key:
                raise ValueError("GEMINI_API_KEY must be set if use_local_llm is False")
    ...
    def _build_rag_messages(self, query: str, retrieved_documents: list) -> list:
        ...
        for i, doc in enumerate(retrieved_documents, 1):
            ...
            if doc_type in ('Image', 'Table'):
                ...
                if not _settings.use_local_llm:
